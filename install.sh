...
if [[ "$TARGET" == "--omarchy" ]]; then
  ...
  if [[ -e "$OMARCHY_PLUGIN_LINK" && ! -L "$OMARCHY_PLUGIN_LINK" ]]; then
    echo "Error: $OMARCHY_PLUGIN_LINK exists and is not a symlink." >&2
    exit 1
  fi
  ln -sfn "$OMARCHY_PLUGIN_DIR" "$OMARCHY_PLUGIN_LINK"
